<template>
	<section>
			        	...
			        	<Row>
		        			...
		        			<Col span="15" offset="1">
		        				 <Select v-model="showonmodal.flowid" style="width:200px" @on-change="changetableflowid(showonmodal.flowid)">
							        <Option v-for="item in flowList" :value="item.flowid" :key="item.flowid">{{item.name}}</Option>
							    </Select>
		        			</Col>
		        		</Row>
		        		...
		        		<Row>
		        			...
		        			<Col span="15" offset="1">
		        				 <Select v-model="showonmodal.nodeid" style="width:200px" @on-change="changetablenodeid(showonmodal.nodeid)">
							        <Option v-for="item in nodeList" :value="item.nodeid" :key="item.nodeid">{{item.nodeid}}</Option>
							    </Select>
		        			</Col>
		        		</Row>
			        </div>
					...
					<div v-show="isViewBtn">
						<Row>
							...
							<Select v-model="viewpageid" style="width:200px;margin-bottom: 15px;">
								<Option v-for="(item,index) in tablePageList" :value="item.pageid" :key="index">{{ item.pagename }}</Option>
							</Select>
							</Col>
						</Row>
					</div>
					...
					<div v-show="showonmodal.iscountBtn">
						<Row>
							...
							<Select v-model="showonmodal.countColumnStart" style="width:200px;margin-bottom: 15px;">
								<Option v-for="(item,index) in countColumnData" :value="item.key" :key="index">{{item.title}}</Option>
							</Select>
							</Col>
						</Row>
					</div>
					<div v-show="showonmodal.iscountBtn">
						<Row>
							...
							<Select v-model="showonmodal.countColumnCalculation" style="width:200px;margin-bottom: 15px;">
								<Option v-for="(item,index) in countColumnCalculations" :value="item.value" :key="index"></Option>
							</Select>
							</Col>
						</Row>
					</div>
				...
				<div style="border: 1px dashed #ddd;padding: 5px;margin-bottom: 15px;">
					...
					<div v-show="showonmodal.isfilterBtn">
						<Row>
							...
							<Col span="17" offset="1">
							<Select v-model="showonmodal.filterkey" style="width:200px;margin-bottom: 15px;">
								<Option v-for="(item,index) in countColumnData" :value="item.key" :key="index">{{item.title}}</Option>
							</Select>
							</Col>
						</Row>
						...
						<div v-show="showonmodal.filterchoosesystemsk">
							<Row>
								...
								<Select v-model="showonmodal.filtervaluebychoose" style="width:200px;margin-bottom: 15px;">
									<Option v-for="item in systemsks" :value="item.key" :key="item.key">{{item.value}}</Option>
								</Select>
								</Col>
							</Row>
						</div>
			...
			<Form>
				...
				<Form-item>
					<div v-show="inputIsInput">
						<Row>
							<Col span="10" style="text-align: right;"> 是否设置输入框
							</Col>
							<Col span="13" offset="1">
							<Checkbox v-model="issetInput">
								<span v-show="!issetInput">不设置输入框</span>
								<span v-show="issetInput">设置输入框的数据来源</span>
							</Checkbox>
							</Col>
						</Row>
					</div>
				...
				<Form-item>
					...
					<div v-show="issetInput">
						...
						<Row>
							...
							<Col span="13" offset="1">
							<Checkbox v-model="setInputValueFromSystem">
								<span v-show="!setInputValueFromSystem">输入框的数据来源是术语</span>
								<span v-show="setInputValueFromSystem">输入框的数据来源是系统关键字</span>
							</Checkbox>
							</Col>
						</Row>
						<!--系统关键字下拉框-->
						...
						<div style="width: 100%;min-height: 20px;" v-show="!setInputValueFromSystem">
							<Row style="padding: 10px 0;">
								...
								<Select v-model="keyofsetInputValue" placeholder="请选择一个术语">
									<Option v-for="item in queryCkids" :value="item.id" :key="item.id">{{ item.value }}</Option>
								</Select>
								</Col>
							</Row>
						</div>
					...
					<div style="width: 100%;min-height: 20px;" v-show="inputIsSelect">
						<Row style="padding: 10px 0;">
							...
							<Select v-model="setInputFlowid" placeholder="" v-on:on-change="selectFlow">
								<Option v-for="word in flowList" :value="word.flowid" :key="word.flowid">{{word.name}}</Option>
							</Select>
							</Col>
						</Row>
						<Row style="padding: 10px 0;">
							...
							<Select v-model="setInputword" placeholder="">
								<Option v-for="word in queryAllSkids" :value="word.id" :key="word.id">{{word.value}}</Option>
							</Select>
							</Col>
						</Row>
					</div>
				...
				<Form-item prop="componet">
					<Row>
						...
						<Select v-model="setDummyForm.dummyComponet" style="width:200px">
							<Option v-for="item in dummyComponents" :value="item.id" :key="item.id">{{ item.value }}</Option>
						</Select>
						</Col>
					</Row>
				</Form-item>
				...
				<Form-item prop="fontWeight">
					<Row>
						...
						<Select v-model="setLabelForm.fontStyle.fontWeight" style="width:200px">
							<Option v-for="item in fontWeights" :value="item.value" :key="item.value"></Option>
						</Select>
						</Col>
					</Row>
				</Form-item>
				<Form-item prop="textAlign">
					<Row>
						...
						<Select v-model="setLabelForm.fontStyle.textAlign" style="width:200px">
							<Option v-for="item in textAligns" :value="item.value" :key="item.value"></Option>
						</Select>
						</Col>
					</Row>
				</Form-item>
						...
						<Row>
							...
							<Row v-show="!isCk">
								...
								<Select v-model="setLabelForm.skid" style="width:200px">
									<Option v-for="item in querySkids" :value="item.id" :key="item.id">{{ item.value }}</Option>
								</Select>
								</Col>
							</Row>
							</Col>
				...
				<Form-item prop="from">
					<Row>
						...
						<Select v-model="setstronglistForm.from" style="width:200px">
							<Option v-for="item in listheads" :value="item.key" :key="item.key">{{item.title}}</Option>
						</Select>
						</Col>
					</Row>
				</Form-item>
		...
		<!--TODO 未来需要上传传递更多的参数-->
		<Modal v-model="setmyuploadmodal" title="设置上传" @on-ok="setmyuploadOk" @on-cancel="cancel">
	        		...
	        		<Row>
	        			...
	        			<Col span="15" offset="1">
	        				 <Select v-model="myuploadForm.foldertype" style="width:200px" v-on:on-change="foldertypechange">
						        <Option v-for="(item,index) in foldertypes" :value="item.type" :key="index">{{item.type}}</Option>
						    </Select>
	        			</Col>
	        		</Row>
	        	</Form-item>
	        		...
	        		<Row>
	        			...
	        			<Col span="15" offset="1">
	        				 <Select v-model="myuploadForm.folderkey" style="width:200px">
						        <Option v-for="(item,index) in flowList" :value="item.flowid" :key="index">{{item.name}}</Option>
						    </Select>
	        			</Col>
	        		</Row>
	        	</Form-item>
...
<script>
	...
	let selectContent = '<Form-item label="选择"><Select v-model="formInline.xuanze" placeholder="请选择"><Option v-for="item in items" v-model="item.id" :key="item.id">{{item.value}}</Option></Select></Form-item>'
	export default {
		components: {},
		data() {
			return {
		...
		methods: {
			...
			setTable() {
				...
				for(var i = 0, l = this.resourceKeys.length; i < l; i++) {
					for(var j = 0, lr = this.resourceData.length; j < lr; j++) {
				...
				//TODO 需要设置列表按钮打开的页面是否在模态框显示
				//获取viewpage  和  editpage            	
				...
				if(this.noAreaIndex != "" && (this.colIndex == "" || this.colIndex == undefined)) {
					pagecontent[this.noAreaIndex].value = value
					pagecontent[this.noAreaIndex].open = open
					pagecontent[this.noAreaIndex].newSetting = JSON.stringify(newTableData)
					pagecontent[this.noAreaIndex].opensk = JSON.stringify(this.showonmodal)
				} else if((this.noAreaIndex == "" || this.noAreaIndex == undefined) && this.colIndex != "") {
					pagecontent[this.colIndex].child[this.colButtonIndex].value = value
					pagecontent[this.colIndex].child[this.colButtonIndex].open = open
					pagecontent[this.colIndex].child[this.colButtonIndex].newSetting = JSON.stringify(newTableData)
								...
								if(inputDragdata[tmp].child[tp].eltype == "input"){
									...
									for(var k = 0, ln = newArr_input.length; k < ln; k++) {
										if(/label="项目"/.test(newArr_input[k])) {
											newArr_input[k] = 'label="' + inputDragdata[tmp].child[tp].label + '"><i-input'
										}
										if(/v-model.trim="formInline.xiangmu"/.test(newArr_input[k])) {
											newArr_input[k] = 'v-model.trim="formInline.' + inputDragdata[tmp].child[tp].elabel + '"'
									...
									for(let z = 0, ld = newArr_date.length; z < ld; z++) {
										if(/label="时间"/.test(newArr_date[z])) {
											newArr_date[z] = 'label="' + inputDragdata[tmp].child[tp].label + '"><Date-picker'
										}
										if(/v-model:value="formInline.shijian"/.test(newArr_date[z])) {
											newArr_date[z] = 'v-model:value="formInline.' + inputDragdata[tmp].child[tp].elabel + '"'
										}
									}
									...
									for(let x = 0, lse = newArr_select.length; x < lse; x++) {
										if(/label="选择"/.test(newArr_select[x])) {
											newArr_select[x] = 'label="' + inputDragdata[tmp].child[tp].label + '"><Select'
										}
										if(/v-model="formInline.xuanze"/.test(newArr_select[x])) {
											newArr_select[x] = 'v-model="formInline.' + inputDragdata[tmp].child[tp].elabel + '"'
										}
