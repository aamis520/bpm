...
<template>
    ...
    <div class="tabform">
        ...
        <Form ref="formInline" :model="formInline" :rules="ruleValidate" :label-width="80">
            <div id = "layouttobeadd"><Row><Col span="12"><Form-item v-show="showformInline.zihangming" prop="zihangming" label="自行名"><i-input :disabled="disabledformInline.zihangming" v-model.trim="formInline.zihangming" placeholder="请输入..."></i-input></Form-item>
</Col><Col span="12"><Form-item v-show="showformInline.zileixing" prop="zileixing" label="子类型"><i-input :disabled="disabledformInline.zileixing" v-model.trim="formInline.zileixing" placeholder="请输入..."></i-input></Form-item>
</Col></Row><Row><Col span="24"><RmyUpload97050665></RmyUpload97050665></Col></Row><Row><Col span="24"><RfileListWithSubmit97053104></RfileListWithSubmit97053104></Col></Row><Row><Col span="24"><RmyFiles97059582></RmyFiles97059582></Col></Row></div>
...
<script>
	...
	let importstart;
 import RmyFiles97059582 from '../../5a1689c03d287b1826b6118d/kaishi/myFiles97059582.vue'
 import RmyUpload97050665 from '../../5a1689c03d287b1826b6118d/kaishi/myUpload97050665.vue'
 import RfileListWithSubmit97053104 from '../../5a1689c03d287b1826b6118d/kaishi/fileListWithSubmit97053104.vue'
	let selectwords=[];//下拉框
...
RmyFiles97059582,
RmyUpload97050665,
RfileListWithSubmit97053104,VueEditor,selectOnePersonModal},
        data() {
            return {
            	//人员选择相关
            	choosepersonkey:'',
...
//    			delCookie("parent")
            ...
            handleSubmit(name) {
				...
				this.$refs[name].validate((valid) => {
					if(valid) {
						//判断当前的提交操作是哪种类型
						/*共有8种情况
							globalconfig中为true的个数为 n
							1，n=2 --必然是用模态框新建子数据
							2，n=1 --(1) 如果cookie种没有parent，这是用模态框新建主数据
									 (2) 如果cookie种没有parent，parent种的pageid与globalconfig的作比较
									 	 相同---用模态框修改主数据
									 	 不同---用模态框新建子数据--这个时候的
							3，n=0   ()url上有itemid，是用页面修改主数据
									 ()url上有parentid，是用页面新建子数据
									 ()url上都没有，是用页面新建主数据
						*/
						//先得到去全局this.globalconfig.showingpagedlgqueue
						var queueData = me.globalconfig.showingpagedlgqueue;
						var lens = queueData.length;
						var pageIsTrue = 0; //queueData中pageid为true的个数
						var pageIsTrueData = []; //queueData中pageid为true的pageid集合
						var parentData = JSON.parse(getCookie("parent"));//得到cookie--parent
						var nowUrl = me.$router.currentRoute.path;//得到url
						for(let i=0;i<lens;i++){
							var keys = Object.keys(queueData[i]);//拿到所有的key
							if(queueData[i][keys[0]]){
								pageIsTrueData.push(queueData[i]);
								pageIsTrue++;
							}
						}
						var submitType = ""; //提交的类型，是新建还是修改  --- new or edit
						var params = {   //提交时用到的参数

						};
						var ifDelCookie = false;//是否删除cookie("parent") -- 对主数据操作后要删掉
						if(pageIsTrue == 2){   //用模态框新建子数据  1
							submitType = "new";
							params["flowid"] = ownflowid;
							params["nodeid"] = getrealnodeid();
							params["usrid"] = window.localStorage.getItem("usrid");
						} else if(pageIsTrue == 1) {
							var keys = Object.keys(pageIsTrueData[0]);
							params["flowid"] = ownflowid;
							params["nodeid"] = getrealnodeid();
							params["usrid"] = window.localStorage.getItem("usrid");
							if(parentData == null){  //用模态框新建主数据 2-1
								submitType = "new";
								ifDelCookie = true;
							} else {  //cookie有parent，
								if(parentData.pageid == keys[0]){  //这个是模态框修改主数据 2-2-1
									submitType = "edit";
									params["itemid"] = parentData.dataid;
									ifDelCookie = true;
								} else {   //这个是模态框新建子数据  2-2-2
									submitType = "new";
									this.formInline.parentid = parentData.dataid;
								}
							}

						} else if(pageIsTrue == 0){
							//从url中拿到flowid，nodeid，在拿到usrid
							var urlParams = me.$router.currentRoute.query;
							params["flowid"] = ownflowid;
							params["nodeid"] = getrealnodeid();
							params["usrid"] = window.localStorage.getItem("usrid");
							if(urlParams.itemid){  //用页面修改主数据 3-1
								submitType = "edit";
								params["itemid"] = urlParams["itemid"];
								ifDelCookie = true;
							} else if(urlParams.parentid){  //用页面新建子数据 3-2
								submitType = "new";
								this.formInline.parentid = urlParams["parentid"];
							} else {  //用页面新建主数据  3-3
								submitType = "new";
								ifDelCookie = true;
							}
						...
						if(submitType == "new"){
							var thisId = "";
							if(me.globalconfig.parentid.length == 1){  //主数据的新建
								thisId = me.globalconfig.parentid[0];
							} else if (me.globalconfig.parentid.length == 2) {
								thisId = me.globalconfig.parentid[1];
								me.formInline.parentid = me.globalconfig.parentid[0];
							}
							me.$http.get(me.globalconfig.submitdataapi, {
								params: {
									usrid: params["usrid"],
							...
							.then((response) => {
								me.globalconfig.localrefresh++;
								var len = me.globalconfig.showingpagedlgqueue.length;
								var ifJumpBack = false;
								if(me.globalconfig.showingpagedlgqueue.length>0){
									ifJumpBack = me.globalconfig.showingpagedlgqueue[len -1].showatmodal;
									me.globalconfig.showingpagedlgqueue.pop();
								}
								if(!ifJumpBack){
									me.$router.go(-1);
								}
								if(ifDelCookie && getCookie("parent")){  //如果应该清楚cookie且有cookie（parent） --就删除掉
									delCookie("parent");
								}
							}, () => {
								me.$Message.success('这是一条失败的提示');
							});
							me.globalconfig.parentid.pop();
							me.globalconfig.localrefresh++;
						} else if(submitType == "edit") {
							me.$http.get(this.globalconfig.updatedataapi, {
								params: {
									usrid: params["usrid"],
									flowid: params["flowid"],
									nodeid: getrealnodeid(),
    ...
    function generateDataIfShowmodel () {
            var itemshow = {"zihangming":"","zileixing":"","submitbtn":"","backbtn":""};
