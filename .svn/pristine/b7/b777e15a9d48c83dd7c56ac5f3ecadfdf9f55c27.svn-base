...
<script>
	...
	let parentid = ""
	let showchilddata;//表单基础设置的选择
	let RelationshipObj = {};//用来存放下拉框关系的
...
//    			delCookie("parent")
        		...
        		if(selectwords){
					if(selectwords instanceof Array) {
						...
						if(len > 0) {
							...
							for(let i = 0; i < len; i++) {
								var ttmp = i
								//下边的分支条件是把各个下拉框的标识和类型放入其中 type = 1--联动 ，2--自动填充，3单纯下拉框   by --- liuguochao
								//这段代码是配合changeselect来用的
								var inputlinkagesArr = JSON.parse(selectwords[i].inputlinkages);//各个下拉代表的联动关系的数组，如果是空数组，没有联动关系-长度为一的otherflowkey和ownflowkey不一样，是联动，其他的是自动填充
								RelationshipObj[selectwords[i].elabel] = inputlinkagesArr;//把这个下拉框的联动关系存到RelationshipObj中
								//得到每条下拉框对应的数据
								...
								//上边的分支条件是把各个下拉框的标识和类型放入其中 type = 1--联动 ，2--自动填充，3单纯下拉框   by --- liuguochao
							}
						}
					}
				}
            ...
            handleSubmit(name) {
				...
				this.$refs[name].validate((valid) => {
					if(valid) {
						for(var key in this.formInline){  //这个循环是将时间格式转换成毫秒数，--可能会不严谨（其他的提交项也有可能是object类型--时间有点赶，下次再调试）
							if(this.formInline[key]){
								if(typeof(this.formInline[key]) == "object"){  //只有时间格式的类型是对象 ----Thu Dec 28 2017 00:00:00 GMT+0800 (中国标准时间)
									this.formInline[key] = this.formInline[key].getTime();
								}
							}
						}
						var parentData = JSON.parse(getCookie("parent"));//得到cookie--parent---这个是肯定有的，没有就是不对的
						var submitType = parentData.type; //提交的类型，是新建还是修改  --- new or edit
						var params = {
							flowid : ownflowid,
						...
						if(submitType = "new"){
							if(ischildflow !== "undefined"){   //子数据的新建
								me.formInline.parentid =  parentData.dataid;
								this.$http.get(this.globalconfig.generatedataidapi,
								{
									params: {
										flowid:ownflowid
								...
								.then((response) => {
									...
									.then((response) => {
										...
										me.$Message.success('新建失败了');
									});	
								}, () => {
									this.$Message.success('这是一条失败的提示');
								});
							} else {  //主数据的新建
								params["id"] = parentData.dataid;
								me.$http.get(me.globalconfig.submitdataapi, {
									params: {
										flowid: params["flowid"],
										nodeid: params["nodeid"],
            ...
            changeselect(key,str){  //liuguochao
				var me = this;
				var keys = str + "s";
				var Relationship = RelationshipObj[str]; //这里边的属性代表的是这恶下拉框的类型
				var data = transitArr[str + "s"];//这个下拉框的所有数据
				var thisData = null;  //当前数据
				//拿到本下拉框当前的数据
				...
				if(Relationship.length > 0){  //证明是有联动或自动填充关系的
					for(let i=0;i<Relationship.length;i++){
						//得到了下拉框有联动的关系
						var give= Relationship[i].otherflowkey;//给值，
						var accept= Relationship[i].ownflowkey;//接受值
						if(give == str){ //相等的是联动的
							var byFillData = transitArr[accept + "s"];
							var tempByFillData = [];
							for(let j=0;j<byFillData.length;j++){
								if(byFillData[j].sk[str] == key){
									tempByFillData.push(byFillData[j]);
								}	
							}
							me[accept + "s"] = tempByFillData;
						} else {
							me.formInline[accept] = thisData["sk"][give];
						}
					}
				}
			},
  ...
  function getrealnodeid(){
	...
	function uniqueArray(arr) {
		...
		for(let i=0;i<arr.length;i++){
			if(tempValue.indexOf(arr[i].value) == -1){
				ret.push(arr[i]	);
				tempValue.push(arr[i].value);
			}
		}
	    return ret;
	}
