					...
					<div class="form" v-show="formShow">
				        <Form id="form" ref="formInline" :model="formInline" :label-width="80">
				        	...
				        	<div style="margin: 15px auto;padding-bottom: 60px;position: relative;">
				        		<Tabs value="tab1" type="card" closable @on-click="tabNameChange" @on-tab-remove="removeTab">
			        	...
			        	<Row>
		        			...
		        			<Col span="15" offset="1">
		        				 <Select v-model="showonmodal.flowid" style="width:200px" @on-change="changetableflowid(showonmodal.flowid)">
							        <Option v-for="item in flowList" :value="item.flowid" :key="item.pageid" :label="item.name"></Option>
							    </Select>
		        			</Col>
		        		</Row>
	        		...
	        		<Row>
	        			...
	        			<Col span="15" offset="1">
	        				<Select v-model="newcountcolumnForm.from" style="width:200px">
						        <Option v-for="(item,index) in newcountcolumnfroms" :value="item.key" :label="item.value" :key="index"></Option>
						    </Select>
	        			</Col>
	        		</Row>
	        	</Form-item>
		        					...
		        					<Row>
		        						<Col span="12">
					        				<Select v-model="item.key" style="margin-bottom: 15px;">
										        <Option v-for="item in countColumnData" :value="item.key" :key="item.key"></Option>
										    </Select>
					        			</Col>
					        			<Col span="12">
		        			...
		        			<Col span="16">
		        				<div v-for="(item,index) in newcountcolumnForm.relate.columns" :key="index">
		        					<Row>
		        						<Col span="12">
					        				<Select v-model="item.key" style="margin-bottom: 15px;">
										        <Option v-for="(item,index) in newcountcolumndata" :label="item.item" :value="item.key" :key="index"></Option>
										    </Select>
					        			</Col>
					        			<Col span="12">
					        				<Select v-model="item.calculation" style="margin-bottom: 15px;">
	        		...
	        		<Row>
	        			...
	        			<Col span="15" offset="1">
	        				 <Select v-model="setDummyForm.dummyComponet" style="width:200px">
						        <Option v-for="item in dummyComponents" :value="item.id" :label="item.value" :key="item.id"></Option>
						    </Select>
	        			</Col>
	        		</Row>
	        	</Form-item>
         	...
         	handleQueryData:function(){
				...
				.then((response)=>{
					if(response.body.error){
    					...
    					//标签回显
    					if(page.tabsstr && page.tabsstr !=""){
    						var tabsstr = JSON.parse(page.tabsstr)
    						this.pageTabs = tabsstr
    					}
    					//组件回显
...
//				//设置完成之后 将所有中间model置空
            ...
            sendData(){
            	...
            	for(var q=0,tabLen = tabs.length;q<tabLen;q++){
            		//拼接页签参数
            		tabStr += tabStart + tabs[q].label + tabLabel + tabs[q].name + importPage + tabs[q].importPageid + tabEnd
            	}
            	tabStr = '<Tabs value="tab1">' + tabStr + '</Tabs>'
            	//传递的参数
            	var newstr = ''
				...
				this.$http.get(this.globalconfig.updateframeapi,{
					params:{
    					...
    					pageid:this.querydata.pageid,
              			...
              			pagename:this.querydata.pagename,
						newlayout:newstr              		
    				}
				},{emulateJSON:true})
				.then((response)=>{
