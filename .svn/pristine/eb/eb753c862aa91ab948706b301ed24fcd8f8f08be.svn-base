<template>
	<section>
				...
				<Card class="ableDrag">
					...
					<Menu style="width: 90%;margin: 0 auto;">
						...
						<Submenu name="5">
							...
							<!--普通列表-->
							<Menu-item name="5-1" id="listdrag" style="min-height: 160px; position: relative; padding: 10px;">
								<div v-for="n in 100" :key="n" class="myList elcss" draggable="true" @dragstart="drag($event)">
									<div class="setting">
										...
										<Button class="delLabel" @click.stop="delThisEl($event)" type="error">删除</Button>
										<Button class="setDummy" @click="setMylist($event)" type="info">设置</Button>
									</div>
								</div>
							</Menu-item>
							<!--strongList-->
							<Menu-item name="5-1" id="stronglistdrag" style="min-height: 80px; position: relative; padding: 10px;">
								<div v-for="(item,index) in searchboxs" :key="index" class="mystrongList elcss" draggable="true" @dragstart="drag($event)">
									<div class="setting">
										<div class="searchbox" style="text-align: right;padding: 15px 0;">
											<Button type="primary" @click="addSearch">添加</Button>
											<Button type="error" @click="delSearch">删除</Button>
											<Input v-for="(search,index) in item.searchbox" :key="search.label" placeholder="请输入..." style="margin: 10px 0;">
						...
						<Submenu name="4">
							...
							<Menu-item name="4-11" id="col1drag" style="min-height: 60px;position: relative;">
							...
							<Menu-item name="4-1" id="col2drag" style="min-height: 60px;position: relative;">
							...
							<Menu-item name="4-2" id="col3drag" style="min-height: 60px;position: relative;">
							...
							<Menu-item name="4-3" id="col4drag" style="min-height: 60px;position: relative;">
							...
							<Menu-item name="4-4" id="col6drag" style="min-height: 60px;position: relative;">
						...
						<Submenu name="1">
							...
							<Menu-item name="1-1" id="buttondrag" style="min-height: 60px; position: relative; padding: 10px;">
								<div class="moni anniu myButton" v-for="(anniu,index) in buttonGroup" draggable='true' @dragstart.capture='drag($event)'>{{anniu.value}}
									<Button class="delthis" @click.stop="delThisEl($event)" type="error" shape="circle" icon="close"></Button>
									<Button class="setThis" @click="setOpen($event)" type="info">设置</Button>
								</div>
							</Menu-item>
						</Submenu>
						<!--dummyUI-->
						<Submenu name="2">
							...
							<Menu-item name="4-1" id="dummyUIdrag" style="min-height: 260px; position: relative; padding: 10px;">
								<div v-for="n in 100" :key="n" class="dummyUI elcss dragBox" draggable="true" @dragstart="drag($event)">
									<div class="setting">
										<img draggable="false" src="../../images/histogram1.png" alt="dummyUI" />
										<Button class="delChart" @click.stop="delThisEl($event)" type="error" shape="circle" icon="close"></Button>
										<Button class="setDummy" @click="setDummy($event)" type="info">设置</Button>
									</div>
								</div>
							</Menu-item>
						</Submenu>
						...
						<Submenu name="6">
							...
							<Menu-item name="6-1" id="carddrag" style="min-height: 260px; position: relative; padding: 10px;">
								<div v-for="n in 100" :key="n" class="myCard elcss" draggable="true" @dragstart="drag($event)">
									<div class="setting">
										<img draggable="false" style="display: block;width: 90%;margin: 0 auto;" src="../../images/card.png" alt="mycard" />
										<Button class="delChart" @click.stop="delThisEl($event)" type="error" shape="circle" icon="close"></Button>
										<Button class="setDummy" @click="setmycard($event)" type="info">设置</Button>
									</div>
								</div>
							</Menu-item>
						</Submenu>
						...
						<Submenu name="3">
							...
							<!--只有上传按钮-->
							<Menu-item name="3-1" id="onlyuploaddrag" style="min-height: 100px; position: relative; padding: 10px;">
								<div v-for="n in 100" :key="n" class=" elcss myuPload" draggable="true" @dragstart="drag($event)">
									<div class="setting">
										<img draggable="false" src="../../images/myupload.png" alt="dummyUI" />
										<Button class="delChart" @click.stop="delThisEl($event)" type="error" shape="circle" icon="close"></Button>
										<Button class="setDummy" @click="setmyupload($event)" type="info">设置</Button>
									</div>
								</div>
							</Menu-item>
							<!--只有上传列表-->
							<Menu-item name="3-1" id="onlyfilesdrag" style="min-height: 100px; position: relative; padding: 10px;">
								<div v-for="n in 100" :key="n" class=" elcss myFiles" draggable="true" @dragstart="drag($event)">
									<div class="setting">
							...
							<!--上传按钮+上传列表-->
							<Menu-item name="3-1" id="uploadfiledrag" style="min-height: 100px; position: relative; padding: 10px;">
								<div v-for="n in 100" :key="n" class=" elcss fileListWithSubmit" draggable="true" @dragstart="drag($event)">
									<div class="setting">
						...
						<Submenu name="3">
							...
							<Menu-item name="3-1" id="piedrag" style="min-height: 360px; position: relative; padding: 10px;">
								<div v-for="n in 100" :key="n" class="pieChart myPie elcss" draggable="true" @dragstart="drag($event)">
									<div class="setting">
										<img draggable="false" style="display: block;width: 80%;margin: 0 auto;" src="../../images/piegram1.png" alt="dummyUI" />
										<Button class="delChart" @click.stop="delThisEl($event)" type="error" shape="circle" icon="close"></Button>
										<Button class="setDummy" @click="setChart($event)" type="info">设置</Button>
									</div>
								</div>
							</Menu-item>
						</Submenu>
						<Submenu name="5">
							...
							<Menu-item name="5-1" id="histodrag" style="min-height: 360px; position: relative; padding: 10px;">
								<div v-for="n in 100" :key="n" class="pieChart myHisto elcss" draggable="true" @dragstart="drag($event)">
									<div class="setting">
						...
						<Submenu name="5">
							...
							<Menu-item name="5-1" id="labeldrag" style="min-height: 30px; position: relative; padding: 10px;">
								<div v-for="n in 100" class="myLabel elcss" draggable="true" :key="n" @dragstart="drag($event)">
									<div class="setting">
										<span>Label</span>
										<Button class="delLabel" @click.stop="delThisEl($event)" type="error">删除</Button>
										<Button class="setDummy" @click="setLabel($event)" type="info">设置</Button>
									</div>
								</div>
							</Menu-item>
						</Submenu>
			...
			<Card>
				...
				<div class="form" v-show="formShow">
					...
					<Form id="form" ref="formInline" :model="formInline" :label-width="80">
						...
						<div class="dropArea" id="dropArea" style="min-height: 200px; padding-top: 40px;border: 1px solid darkolivegreen;padding-bottom: 40px;margin: 15px 10px;" @drop.stop='drop($event)' @dragover='allowDrop($event)'>
						...
						<Row style="padding: 10px 0px;padding-right: 5px; border: 1px dashed #ddd;margin-bottom: 10px;" v-for="(item,index) in newautoformset.actions" :key="index">
							<Col span="12">
								...
								<Row>
				        			...
				        			<Col span="9" offset="1">
				        				<Select v-model="item.type">
									        <Option v-for="(target,tindex) in nowallsks" :label="target.value" :value="target.label" :key="tindex"></Option>
									    </Select>
				        			</Col>
				        		</Row>
			        		</Col>
		...
		<!--智能表单-->
		<!--设置表头-->
		<Modal v-model="addTitleModal" width="900" title="添加表格表头" @on-ok="setTable" @on-cancel="cancelTable">
			<Row>
				...
				<div>
					...
					<div v-show="isViewBtn">
						<Row>
							<Col span="6" style="font-size: 14px;line-height: 30px;"> 选择页面
							</Col>
							<Col span="17" offset="1">
								<Select v-model="viewpageid" style="width:200px;margin-bottom: 15px;">
									<Option v-for="(item,index) in tablePageList" :label="item.pagename" :value="item.pageid" :key="index"></Option>
								</Select>
							</Col>
						</Row>
						<Row>
					...
					<div v-show="isEditBtn">
						<Row>
							...
							<Select v-model="editpageid" style="width:200px;margin-bottom: 15px;">
								<Option v-for="(item,index) in tablePageList" :value="item.pageid" :label="item.pagename" :key="index"></Option>
							</Select>
							</Col>
						</Row>
						<Row>
				...
				<div>
					...
					<div v-show="showonmodal.iscountBtn">
						<Row>
							<Col span="6" style="font-size: 14px;line-height: 30px;"> 设置统计列表头：
							</Col>
							<Col span="17" offset="1">
					...
					<div v-show="showonmodal.iscountBtn">
						<Row>
							<Col span="6" style="font-size: 14px;line-height: 30px;"> 选择主列
							</Col>
							<Col span="17" offset="1">
							<Select v-model="showonmodal.countColumnStart" style="width:200px;margin-bottom: 15px;">
								<Option v-for="(item,index) in countColumnData" :value="item.key" :label="item.title" :key="index"></Option>
							</Select>
							</Col>
						</Row>
					</div>
					<div v-show="showonmodal.iscountBtn">
						<Row>
							<Col span="6" style="font-size: 14px;line-height: 30px;"> 选择计算方式
							</Col>
							<Col span="17" offset="1">
							<Select v-model="showonmodal.countColumnCalculation" style="width:200px;margin-bottom: 15px;">
								<Option v-for="(item,index) in countColumnCalculations" :value="item.value" :label="item.value" :key="index"></Option>
							</Select>
							</Col>
						</Row>
					</div>
					<div v-show="showonmodal.iscountBtn">
						<Row>
							<Col span="6" style="font-size: 14px;line-height: 30px;"> 选择被动列
							</Col>
							<Col span="17" offset="1">
				...
				<div style="border: 1px dashed #ddd;padding: 5px;margin-bottom: 15px;">
					...
					<div v-show="showonmodal.isfilterBtn">
						<Row>
							<Col span="6" style="font-size: 14px;line-height: 30px;"> 选择过滤条件列
							</Col>
							<Col span="17" offset="1">
						...
						<div v-show="showonmodal.filterchoosesystemsk">
							<Row>
								<Col span="9" style="font-size: 14px;line-height: 30px;"> 选择系统关键字
								</Col>
								<Col span="14">
								<Select v-model="showonmodal.filtervaluebychoose" style="width:200px;margin-bottom: 15px;">
									<Option v-for="(item,index) in systemsks" :value="item.key" :label="item.value" :key="index"></Option>
								</Select>
								</Col>
							</Row>
						</div>
						<div v-show="!showonmodal.filterchoosesystemsk">
							<Row>
								<Col span="9" style="font-size: 14px;line-height: 30px;"> 输入过滤条件的值
								</Col>
								<Col span="14">
		...
		<!--设置输入框的label-->
		<Modal v-model="setLabelModal" title="设置文本输入框" @on-ok="setLabelModalOk()" @on-cancel="cancelInput">
			<Form>
				<Form-item>
					<Row>
						...
						<div style="width: 100%;min-height: 20px;" v-show="setInputValueFromkeyword">
							<Row style="padding: 10px 0;">
								...
								<Select v-model="keyofsetInputValue" placeholder="请选择一个术语">
									<Option v-for="(item,index)  in queryCkids" :value="item.id" :label="item.value" :key="index"></Option>
								</Select>
								</Col>
							</Row>
						</div>
				...
				<Form-item v-show="inputIsSelect">
					...
					<div style="width: 100%;min-height: 20px;">
						<Row style="padding: 10px 0;">
							...
							<Select v-model="setInputFlowid" placeholder="" v-on:on-change="selectFlow">
								<Option v-for="(word,index)  in flowList" :value="word.flowid" :label="word.name" :key="index"></Option>
							</Select>
							</Col>
						</Row>
						<Row style="padding: 10px 0;">
					...
					<div style="padding: 20px 0;border: 1px dashed #ddd;">
						<div style="padding: 5px 0;" v-for="(inputlinkage,index) in inputlinkages" :key="index">
							<Row>
								<!--当前表单的某一项-->
								<Col span="10">
									<Select v-model="inputlinkage.ownflowkey" clearable>
							            <Option v-for="(option,index) in nowallsks" :label="option.value" :value="option.label" :key="index">
								...
								<Col span="10" offset="4">
									<Select v-model="inputlinkage.otherflowkey" placeholder="">
										<Option v-for="(otherflowkey,index) in inputlinksks" :value="otherflowkey.key" :label="otherflowkey.value" :key="index"></Option>
									</Select>
								</Col>
							</Row>
							...
							<Row>
								...
								<Col span="10" offset="4">
									<Select v-model="program.personinfokey" placeholder="">
										<Option v-for="(person,index) in personinfos" :value="person.key" :label="person.label" :key="index"></Option>
									</Select>
								</Col>
							</Row>
			...
			<Form ref="countinputform" :model="countinputform" :label-width="80" class="form">
				<Form-item
		        	...
		        	<Select v-model="countinputsk.sk" style="width:80px;" clearable>
			            <Option v-for="(option,index) in nowallsks" :value="option.label" :label="option.value" :key="index">
			                {{option.value}}
			            </Option>
		            </Select>
		        	<!--加减乘除-->
		...
		<!--设置按钮-->
		<Modal v-model="showButtonModal" title="设置按钮" @on-ok="setButtonOk" @on-cancel="cancel">
			<Form ref="setButtonForm" :model="setButtonForm" :rules="setButtonRuleInline">
				<Form-item prop="buttonName">
					<Row>
						<Col span="8" style="text-align: right;"> 请输入按钮要展示的字：
						</Col>
						<Col span="15" offset="1">
				...
				<Form-item prop="flowid">
					<Row>
						<Col span="8" style="text-align: right;"> 请选择一个流程：
						</Col>
						<Col span="15" offset="1">
				...
				<Form-item prop="nodeid">
					<Row>
						<Col span="8" style="text-align: right;"> 请选择一个节点：
						</Col>
						<Col span="15" offset="1">
				...
				<Form-item prop="openpageid">
					<Row>
						<Col span="8" style="text-align: right;"> 请选择按钮要打开的页面：
						</Col>
						<Col span="15" offset="1">
						<Select v-model="setButtonForm.openPageid" style="width:200px">
							<Option v-for="(item,index) in pageList" :value="item.pageid" :label="item.pagename" :key="index"></Option>
						</Select>
						</Col>
					</Row>
				</Form-item>
				<Form-item prop="showInModal">
					<Row>
						<Col span="8" style="text-align: right;"> 新页面显示：
						</Col>
						<Col span="15" offset="1">
				...
				<Form-item prop="modaltitle" v-show="setButtonForm.showInModal">
					<Row>
						<Col span="8" style="text-align: right;"> 模态框标题设置：
						</Col>
						<Col span="15" offset="1">
		...
		<!--设置dummyUI-->
		<Modal v-model="showDummyModal" title="选择控件" @on-ok="setDummyOk" @on-cancel="cancel">
			<Form ref="setDummyForm" :model="setDummyForm" :rules="setDummyRuleInline">

				<Form-item prop="componet">
					<Row>
						<Col span="8" style="text-align: right;"> 请选择控件：
						</Col>
						<Col span="15" offset="1">
						<Select v-model="setDummyForm.dummyComponet" style="width:200px">
							<Option v-for="(item,index) in dummyComponents" :value="item.id" :label="item.value" :key="index">{</Option>
						</Select>
						</Col>
					</Row>
				</Form-item>
		...
		<!--设置图表-->
		<Modal v-model="showSetChartModal" title="设置图表" @on-ok="setChartOk" @on-cancel="cancel">
			<Form ref="setChartForm" :model="setChartForm" :rules="setChartRuleInline">

				<Form-item prop="ckid">
					<Row>
						<Col span="8" style="text-align: right;"> 请选择术语：
						</Col>
						<Col span="15" offset="1">
		...
		<!--设置Label-->
		<Modal v-model="showSetLabelModal" width="900" title="设置Label" @on-ok="setLabelOk()" @on-cancel="cancel">
			<Form ref="setLabelForm" :model="setLabelForm" :rules="setLabelRuleInline">
				<Form-item prop="fontSize">
					<Row>
						<Col span="8" style="text-align: right;"> 请设置字体大小：
						</Col>
						<Col span="15" offset="1">
				...
				<Form-item prop="fontWeight">
					<Row>
						<Col span="8" style="text-align: right;"> 请设置字体是否加粗：
						</Col>
						<Col span="15" offset="1">
						<Select v-model="setLabelForm.fontStyle.fontWeight" style="width:200px">
							<Option v-for="(item,index) in fontWeights" :label="item.value" :value="item.value" :key="index"></Option>
						</Select>
						</Col>
					</Row>
				</Form-item>
				<Form-item prop="textAlign">
					<Row>
						<Col span="8" style="text-align: right;"> 请设置字体是否居中：
						</Col>
						<Col span="15" offset="1">
						<Select v-model="setLabelForm.fontStyle.textAlign" style="width:200px">
							<Option v-for="(item,index) in textAligns" :label="item.value" :value="item.value" :key="index"></Option>
						</Select>
						</Col>
					</Row>
				</Form-item>
				<Form-item prop="textAlign">
					<Row>
						<Col span="8" style="text-align: right;"> 请设置字体label的来源：
						</Col>
						<Col span="15" offset="1">
				...
				<Form-item prop="labeldesc" v-show="!setLabelForm.chooseSkorck">
					<Row>
						<Col span="8" style="text-align: right;"> 请设置label内容：
						</Col>
				...
				<Form-item prop="ckid" v-show="setLabelForm.chooseSkorck">
					<Radio-group v-model="ckOrskId" style="width: 100%;" v-on:on-change="selectSkOrCk">
						<Row>
							...
							<Row v-show="isCk">
								<Col span="5" style="text-align: left;"> 请选择术语：
								</Col>
								<Col span="15" offset="1">
							...
							<Row v-show="!isCk">
								<Col span="5" style="text-align: left;"> 请选择数据：
								</Col>
								<Col span="15" offset="1">
								<Select v-model="setLabelForm.skid" style="width:200px">
									<Option v-for="(item,index) in querySkids" :value="item.id" :label="item.value" :key="index"></Option>
								</Select>
								</Col>
							</Row>
							</Col>
		...
		<!--stronglist-->
		<Modal v-model="showsetstronglistmodal" title="设置列表" @on-ok="setstronglistOk" @on-cancel="cancel">
			<Form ref="setstronglistForm" :model="setstronglistForm" :rules="setstronglistRuleInline">
				<Form-item prop="label">
					<Row>
						<Col span="8" style="text-align: right;"> Label：
						</Col>
						<Col span="15" offset="1">
				...
				<Form-item prop="from">
					<Row>
						<Col span="8" style="text-align: right;"> From：
						</Col>
						<Col span="15" offset="1">
						<Select v-model="setstronglistForm.from" style="width:200px">
							<Option v-for="(item,index) in listheads" :value="item.key" :label="item.title" :key="index"></Option>
						</Select>
						</Col>
					</Row>
				</Form-item>
		...
		<!--myCard-->
		<Modal v-model="showsetmycardmodal" title="设置卡片" @on-ok="setmycardOk" @on-cancel="cancel">
			<Form ref="mycardForm" :model="mycardForm" :rules="mycardRuleInline">
				<Form-item prop="title">
					<Row>
						<Col span="8" style="text-align: right;"> CardTitle：
						</Col>
						<Col span="15" offset="1">
				...
				<Form-item prop="flowid">
					<Row>
						<Col span="8" style="text-align: right;"> 请选择流程：
						</Col>
				...
				<Form-item prop="nodeid">
					<Row>
						<Col span="8" style="text-align: right;"> 请选择节点：
						</Col>
						<Col span="15" offset="1" :key="003">
						<Select v-model="mycardForm.nodeid" style="width:200px" @on-change="nodeidchange(mycardForm.nodeid)">
							<Option v-for="(item,index) in nodeList" :value="item.nodeid" :label="item.nodeid" :key="index"></Option>
						</Select>
						</Col>
					</Row>
				</Form-item>
				<Form-item prop="pageid">
					<Row>
						<Col span="8" style="text-align: right;"> 选择页面：
						</Col>
						<Col span="15" offset="1">
...
<script>
	...
	let dateContent = '<Form-item label="时间"><Date-picker type="date" placeholder="选择日期" format="yyyy年MM月dd日" v-model.trim="formInline.shijian" ></Date-picker></Form-item>'
	let selectContent = '<Form-item label="选择"><Select v-model.trim="formInline.xuanze" placeholder="请选择"><Option v-for="(item,index) in items" v-model="item.id" :key="item.value" :label="item.value"></Option></Select></Form-item>'
	...
	export default {
		...
		methods: {
			...
			pageoutcol(arr, temp) {
				var huixianout = jQuery('#dropArea')
				var indom = null
				...
				this.$http.get(this.globalconfig.getonepageapi, {
					...
					.then((response) => {
						if(response.body.error) {
							...
							var end = new Date().getTime()
							var runtime = end - start
							console.log("回显UItart"+ runtime/1000 +"s" )
						}
					}, (response) => {
						this.$Message.success('成功');
					})
...
//					})
							...
							this.$http.get(this.globalconfig.getallpagesapi, {
								...
								.then((response) => {
									if(response.body.error) {
										...
										var end = new Date().getTime()
										var runtime = end - start
										console.log("获取所有flow 和nodes"+ runtime/1000 +"s" )
									}
								}, (response) => {
									this.$Message.success('成功');
								})
							...
							this.$http.get(this.globalconfig.getallsksapi, {
								...
								.then((response) => {
									if(response.body.error) {
										...
										var end = new Date().getTime()
										var runtime = end - start
										console.log("获取当前系统所有简单关键字"+ runtime/1000 +"s" )
									}
								}, (response) => {
									this.$Message.success('成功');
								})
				...
				this.$http.get(this.globalconfig.listcksapi, {
					...
					.then((response) => {
						if(response.body.error) {
							...
							var end = new Date().getTime()
							var runtime = end - start
							console.log("获取所有复杂关键字"+ runtime/1000 +"s" )
						}
					}, (response) => {
						this.$Message.success('成功');
					})
				...
				this.$http.get(this.globalconfig.getsksapi, {
					...
					.then((response) => {
						if(response.body.error) {
							...
							var end = new Date().getTime()
							var runtime = end - start
							console.log("获取本流程所有简单关键字"+ runtime/1000 +"s" )
						}
					}, (response) => {
						this.$Message.success('成功');
					})
			...
			changeLabel(index) {
				...
				this.setInputFlowid = this.querydata.flowid
			...
			selectFlow() {
				for(let i = 0, lf = this.flowList.length; i < lf; i++) {
					if(this.setInputFlowid == this.flowList[i].flowid) {
						...
						this.inputlinksks = this.queryAllSkids
						console.log(this.queryAllSkids)
					}
				}
			...
			setinputfromcountOk(){
				console.log(this.countinputform.countinputsks)
			},
			addcountinputsk(){
			...
			setLabelModalOk() {
				console.log(this.inputlinkages)
				//判断类型
				if(this.setvalue == '') {
					...
					if(this.setInputType == "select") {
						...
						inputDragdata[this.inputcolindexindata].child[this.inputincolindex].relateArr = this.queryAllSkids
				...
				if(/dropInputArea/.test(dropAreaClass)){
					...
					for(var i=0,len=dropinputindexs.length;i<len;i++){
						...
						if(corprationIndex == dropinputindexs[temp]){
							console.log(corprationIndex)
							console.log(dropinputindexs[temp])
			...
			sendData() {
				...
				console.log(this.autoformsetdatas)
//				return false;
				//保存sks
				//console.log(sks)
				//console.log(inputsets)
				console.dir(selectwords)
				//传递的参数
				var newstr = ''
				...
				pagecontentstr =inputsstr + newstr
				//更新page  和 pageUI
				//form组件UI
	  					...
	  					this.$http.get(this.globalconfig.updatepageapi,{
			    				params:{
			    					...
			    					pageid:this.querydata.pageid,
			              			...
			              			inputsui:inputsui,
			              			isshowchilddata:this.formbaseset.isshowchilddata,  //表单是否设置子数据
