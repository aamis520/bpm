    ...
    export default {
        ...
        watch:{
			...
			pageishowwinthinmodal(){
				...
				if(len > 0) {
					...
					if((showmodal.filterkey && showmodal.filtervaluebychoose == "system-myitemid") || showmodal.isonlyshowchilddata){
						this.parentid = parentid;
						this.handleQuery();
					}
					let leng = this.globalconfig.showingpagedlgqueue.length;
					for(let m=0;m<leng;m++){
						if(this.globalconfig.showingpagedlgqueue[m][ownpageid]){
							if(this.globalconfig.showingpagedlgqueue[m].vieworedit == "edit"){
            					this.showeditpage = true;
							} else if(this.globalconfig.showingpagedlgqueue[m].vieworedit == "view"){
        ...
        methods:{
        	handleQuery() {
				...
				if(filterValue == "system-myitemid"){
					if(getCookie("parent")){
						var itemsid = JSON.parse(getCookie("parent")).dataid;
					}
				}
                ...
                this.$http.get(this.globalconfig.listsks,
                {
                    params: {
						...
						itemsid:itemsid,
						parentcolname:showmodal.parentcolname
                    }
                }, {emulateJSON: true})
                .then((response) => {
						...
						//只显示子数据
						if(showmodal.isonlyshowchilddata == true) {
							var cookieData = JSON.parse(getCookie("parent"));
							if(cookieData != null){
								var tempData = [];
								for(let i=0;i<tableData.length;i++){
									if(tableData[i].parentid == cookieData.dataid){
										tempData.push(tableData[i]);
									}
								}
								tableData = tempData;
							...
							for(var e=0;e<lnccl;e++){
								...
								if(newcountcolumns[temp].isuse == "true" || newcountcolumns[temp].isuse == true) {
									if(newcountcolumns[temp].from == "own") {
										...
										var relateTemp = temp
										//先请求子流程数据
										setTimeout((function (relateTemp) {
											...
											var zhuColName = newcountcolumns[relateTemp].key
											console.log(relateTemp)
											me.$http.get(me.globalconfig.listsks, {
												params: {
													usrid: window.localStorage.getItem("usrid"),
													flowid: childflowid,
													isshowflowdata: true
											...
											.then((response) => {
												...
												if(columnsbycolumnLen != 0) {
													//各个相关列的数据存放
													var eachColData = {};//各个列的列名和计算方式
													for(let x=0;x<columnsbyrelate.length;x++){  
														var thisColName = columnsbyrelate[x].key;   //当前列的列名--bianhao
														var thisColCalculation =  columnsbyrelate[x].calculation; //当前列的计算方法  +
														eachColData[thisColName] = thisColName;
														eachColData[thisColName + "Condition"] = thisColCalculation;
													}
													var allKeyData = {};//数据按指定值分类
													var middleData = {};//这里存的是每个分类的每个列的和
													for(let y=0;y<childtableData.length;y++){
														if(allKeyData[childtableData[y][childtagkey]]){
															allKeyData[childtableData[y][childtagkey]].push(childtableData[y]);
														} else {
															middleData[childtableData[y][childtagkey]] = {};
															allKeyData[childtableData[y][childtagkey]] = [];
															allKeyData[childtableData[y][childtagkey]].push(childtableData[y]);
														}
													}
													for(var key1 in eachColData){
														if(!(/[+*/-]/.test(eachColData[key1])) || eachColData[key1].length > 1){
															var nowkey = eachColData[key1];
															for(var key2 in allKeyData){ 
																var nowData = allKeyData[key2];
																var tempResult = 0;
																for(let z=0;z<nowData.length;z++){
																	tempResult += Number(nowData[z][nowkey]);
																}
																if(tempResult >= 0){
																	middleData[key2][key1] = tempResult;	
																} else {
																	middleData[key2][key1] = nowData.length;
																}
															}
														}
													}
													var length = columnsbyrelate.length;
													var lastData = {};
													if(length > 1){
														for(let m=0;m<columnsbyrelate.length-1;m++){
															var lastValue = "";
															for(var key3 in middleData){
																var thisClacData = middleData[key3];
																var nowkey = columnsbyrelate[m]['key'];
																var nowclac = columnsbyrelate[m]['calculation']
																var nextkey = columnsbyrelate[m+1]['key'];
																if(nowclac == "+"){
																	lastValue = Number(thisClacData[nowkey]) + Number(thisClacData[nextkey]);
																} else if(nowclac == "-"){
																	lastValue = Number(thisClacData[nowkey]) - Number(thisClacData[nextkey]);
																} else if(nowclac == "*"){
																	lastValue = Number(thisClacData[nowkey]) * Number(thisClacData[nextkey]);
																...
																lastData[key3] = lastValue;
															}
														}
														for(let s=0;s<tableData.length;s++){
															var zhuKeyValue = tableData[s][maintagkey]; //主数据对应的maintagkey的值
															if(lastData[zhuKeyValue] >= 0){
																tableData[s][zhuColName] = lastData[zhuKeyValue];
															} else {
																tableData[s][zhuColName] = 0;
															}	
														}
													} else {
														var thisKey = columnsbyrelate[0]["key"];
														for(let s=0;s<tableData.length;s++){
															var zhuKeyValue = tableData[s][maintagkey]; //主数据对应的maintagkey的值
															if(middleData[zhuKeyValue] && middleData[zhuKeyValue][thisKey] >= 0){
																tableData[s][zhuColName] = middleData[zhuKeyValue][thisKey];
															} else {
																tableData[s][zhuColName] = 0;
															}	
														}
													}
													me.pageCount = tableData.length;
													me.data = me.showTable();
												}
											}, () => {
						...
						if(isapprovalstatus == true || showmodal.filterchoosesystemsk == true) {
							this.$http.get(this.globalconfig.listallapprovalbyids, {
								...
								.then((response) => {
									...
									//begin add by liuguochao 	 //选择我审批的--筛选--shenpiuserid == usrid
									var userid = window.localStorage.getItem("usrid");
									var temphistories = [];
									if(appropersonmyself){
										var tempitemid = "";//临时id为空
										for(let i=0;i<histories.length;i++){
												if(histories[i].shenpiuserid == userid){
													tempitemid = histories[i].itemid;
													temphistories.push(histories[i]);
												}
												if((histories[i].itemid == tempitemid) && (showmodal.approvalconditions == "system-finished") && (histories[i].shenpistatus == "finished")){
													temphistories.push(histories[i]);
													tempitemid = "";
												}
												if((histories[i].itemid == tempitemid) && (showmodal.approvalconditions == "system-deny") && (histories[i].shenpistatus == "deny")){
													temphistories.push(histories[i]);
													tempitemid = "";
												}
											}
									...
									//end add by liuguochao 	 //选择我审批的--筛选--shenpiuserid == usrid
									var alldatas = []
            ...
            showTable(){
            	...
            	if(len>10){
					var pageindex = this.nowPage
	            	for(let i=(this.nowPage-1)*10;i<((this.nowPage)*10);i++){
						if(tableData[i]){
	                ...
	                .then((response) => {
	                    ...
	                    if(this.path != undefined){
		                    ...
		                    if(flag == "view"){
								...
								this.itemData = this.data[index];
								console.log(showmodal.editBtnFunction)
								if(showmodal.editBtnFunction == "update"){
									var objtemp = {}
									objtemp.pageid = ownpageid;
									objtemp.dataid = itemid;
									objtemp.type = "edit";
									...
									this.globalconfig.dataid = itemid
									setCookie('parent', objstr);
								} else if(showmodal.editBtnFunction == "submit"){
